...
var globeObj = (function() {
    ...
    // 获取position
    function getPosition(lng, lat, alt) {//纬度，经度，高度（一种地图格式）=>转换为经纬度高度
    ...
    function plane() {
        ...
        var clientBounds = [52.793056,72.427908,2.970897,135.181814];//??界限 什么鬼

        // 连接
        socket.on('connect', function() {
            socket.emit("sub", clientBounds, -1, '', function(){});//socket发射
        });
        ...
        planeShape.lineTo(-0.2, -1.3);
        planeShape.lineTo(-0.2, -0.2);
        var planeGeometry = new THREE.ShapeGeometry(planeShape);//画个飞机
        // 飞机材质
        ...
        // 时间段
        var curTime = Date.parse(new Date())/1000;//解析一个日期时间字符串,并返回 1970/1/1 午夜距离该日期时间的毫秒数  返回当前时间距离1970/1/1的秒数
